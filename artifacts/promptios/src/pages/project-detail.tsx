...
export default function ProjectDetail() {
  ...
  const handleGenerate = async () => {
    ...
    // Invalidate immediately to show generating status if server updates quickly
          ...
          for (const line of text.split("\n")) {
            if (line.trim().startsWith("data: ")) {
              try {
                ...
                // If files are arriving or status updating, we could aggressively invalidate
                // but doing it at the end is safer
                if (event.done) {
        ...
        <header className="h-14 border-b border-border bg-card/80 backdrop-blur flex items-center justify-between px-4 shrink-0">
          ...
          <div className="flex items-center gap-2">
            ...
            <Button 
              ...
              disabled={isGenerating || project?.status === 'generating' || isLoadingProject}
            ...
            <div className="flex-1 overflow-y-auto py-2">
              {isLoadingFiles ? (
                ...
                <div className="px-4 py-8 text-center text-sm text-muted-foreground font-mono">
                  {project?.status === 'generating' ? 'Awaiting output stream...' : 'No files generated.'}
                </div>
              ) : (
            ...
            {(isGenerating || project?.status === 'generating') && (
              <div className="absolute inset-0 bg-background/50 backdrop-blur-sm z-20 flex flex-col items-center justify-center">
                ...
                <p className="text-sm font-mono text-muted-foreground mt-2 max-w-md text-center">
                  Parsing blueprint logic. Compiling {project?.framework} views and data models...
                </p>
              </div>
            )}
